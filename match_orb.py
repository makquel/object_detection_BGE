...
def filter_distance(matches):
    ...
    print ("#selected matches:{} (out of {})" .format(len(sel_matches), len(matches)))
    return sel_matches
...
img_scene = cv2.imread(img1_path, 0) # cv2.CV_LOAD_IMAGE_GRAYSCALE
...
print ("#keypoints in image1: {}, image2: {}" .format(len(kp_scene), len(kp_ftr)))

# descriptors
k_scene, d_scene = descriptor.compute(img_scene, kp_scene)
k_ftr, d_ftr = descriptor.compute(img_ftr, kp_ftr)

print ("#keypoints in image1: {}, image2: {}" .format(len(d_scene), len(d_ftr)))
